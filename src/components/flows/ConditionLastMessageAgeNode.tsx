...
export default function ConditionLastMessageAgeNode({ id, data, isConnectable }: any) {
  const { t } = useLanguage();
  ...
  return (
    <Card className="w-[260px] border-0 shadow-md rounded-md overflow-hidden ring-1 ring-slate-200 dark:ring-slate-700 bg-white dark:bg-slate-900">
      ...
      <CardHeader className="p-2 pb-1.5 bg-blue-50/50 dark:bg-blue-900/10 border-b border-blue-100 dark:border-blue-900/20">
        <CardTitle className="text-xs font-semibold flex items-center gap-1.5 text-blue-700 dark:text-blue-300">
          ...
          {t('flows.ifLastMessageOlderThan')}
        </CardTitle>
      </CardHeader>

      <CardContent className="p-2 space-y-1.5">
        <div
          className="nodrag nopan nowheel flex items-center gap-1.5"
        ...
        >
          <Input
            ...
            placeholder="24"
            className="text-xs h-7 w-[60px] bg-slate-50 dark:bg-slate-950 border-slate-200 focus-visible:ring-blue-500"
          />
          <Select
            value={data.unit || 'hours'}
            onChange={(e) => handleUnitChange(e.target.value)}
            className="text-xs !h-7 !py-0 w-[120px] bg-slate-50 dark:bg-slate-950 border-slate-200 focus-visible:ring-blue-500"
          >
            <option value="hours">{t('flows.hours')}</option>
            <option value="minutes">{t('flows.minutes')}</option>
          </Select>
        </div>
