...
export default function GroupsCampaignStatsTab() {
  ...
  useEffect(() => {
    ...
    api.stats.getGroupCampaignsList().then(setCampaigns).catch(() => {});
  }, []);

  const loadStats = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.stats.getGroupCampaignStats(startDate, endDate, accountId || undefined, campaignId || undefined);
      setResult(data);
    } catch (error) {
      console.error('Failed to load group campaign stats:', error);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const filePath = await api.stats.exportReport('group_campaign', startDate, endDate, accountId || undefined, campaignId || undefined);
      if (filePath) {
  ...
  const columns: StatsTableColumn[] = [
    ...
    { key: 'group_name', label: language === 'he' ? 'קבוצה' : language === 'ar' ? 'المجموعة' : 'Group', render: (r) => r.group_name || r.group_id },
    { key: 'account_name', label: language === 'he' ? 'חשבון' : language === 'ar' ? 'الحساب' : 'Account', render: (r) => r.account_name || r.account_phone || '-' },
    {
      key: 'status',
    ...
    { key: 'run_date', label: language === 'he' ? 'תאריך הרצה' : language === 'ar' ? 'تاريخ التشغيل' : 'Run Date' },
    {
      key: 'sent_at',
