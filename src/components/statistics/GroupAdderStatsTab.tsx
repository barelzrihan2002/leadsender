...
export default function GroupAdderStatsTab() {
  ...
  useEffect(() => {
    ...
    api.stats.getCampaignsList('group_adder').then(setCampaigns).catch(() => {});
  }, []);

  const loadStats = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.stats.getGroupAdderStats(startDate, endDate, accountId || undefined, campaignId || undefined);
      setResult(data);
    } catch (error) {
      console.error('Failed to load group adder stats:', error);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const filePath = await api.stats.exportReport('group_adder', startDate, endDate, accountId || undefined, campaignId || undefined);
      if (filePath) {
  ...
  const columns: StatsTableColumn[] = [
    ...
    { key: 'account_name', label: language === 'he' ? 'בוצע ע"י' : language === 'ar' ? 'بواسطة' : 'Performed By', render: (r) => r.account_name || r.account_phone || '-' },
    {
      key: 'status',
    ...
    { key: 'result_code', label: language === 'he' ? 'תוצאה' : language === 'ar' ? 'النتيجة' : 'Result Code', render: (r) => r.result_code || '-' },
    {
      key: 'sent_at',
