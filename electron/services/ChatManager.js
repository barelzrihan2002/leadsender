...
export class ChatManager {
     ...
     */
    async findOrCreateChat(phoneNumber, accountId, whatsappClient, senderName) {
        ...
        for (const variant of variants) {
            existingChat = this.db.prepare(`SELECT id, contact_id, photo, name FROM chats WHERE phone_number = ? AND account_id = ?`).get(variant, accountId);
            if (existingChat)
                break;
        }
        if (existingChat) {
            ...
            if (!existingChat.name && senderName) {
                ...
                const contact = this.db.prepare(`SELECT name FROM contacts WHERE id = ?`).get(existingChat.contact_id);
        ...
        let contact = null;
        for (const variant of variants) {
            contact = this.db.prepare(`SELECT id, name FROM contacts WHERE phone_number = ?`).get(variant);
            if (contact)
                break;
        }
        ...
        if (!contact && normalizedNumber.length >= 9) {
            ...
            contact = this.db.prepare(`SELECT id, name FROM contacts WHERE phone_number LIKE '%' || ?`).get(last9);
        }
        if (!contact) {
            // Create new contact
            const contactId = uuidv4();
            const contactName = senderName || null;
            this.db.prepare(`INSERT INTO contacts (id, phone_number, name) VALUES (?, ?, ?)`).run(contactId, normalizedNumber, contactName);
     ...
     */
    updateLastMessageAt(chatId, timestamp) {
     ...
     */
    async downloadAndSavePhoto(chatId, phoneNumber, client) {
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                protocol.get(profilePicUrl, (response) => {
                    // Follow redirects
