...
export class CampaignScheduler {
    constructor(db, whatsappManager, duoplusManager) {
     ...
     */
    async onAccountReady(accountId) {
        try {
        ...
        AND ca.account_id = ?
      `);
            const runningCampaigns = stmt.all(accountId);
    ...
    resumeRunningCampaigns() {
      ...
      `);
            const runningCampaigns = stmt.all();
            if (runningCampaigns.length > 0) {
    ...
    logActivity(type, message, relatedId) {
        try {
            const stmt = this.db.prepare(`
     ...
     */
    async sendViaCloudPhone(accountId, phoneNumber, message) {
        ...
        if (!verified) {
            throw new Error('Could not verify the message was actually sent from the cloud phone - no new outgoing message ' +
                'was detected in WhatsApp Web for this chat. The tap/keyevent may have missed the send button.');
        }
    }
    markRemainingPendingContactsAsFailed(campaignId, error, resultCode) {
        const stmt = this.db.prepare(`
    ...
    async getEligibleAccountsForCampaign(campaignId, campaign, accountIds) {
        if (!this.isGroupAdderCampaign(campaign)) {
            return accountIds;
        }
    ...
    async handleGroupAdderContact(campaignId, accountId, contact, state, campaign) {
        if (!campaign.target_group_id) {
      ...
      `);
        ...
        if (!result.success) {
            console.warn(`⚠️ Group adder failed for account ${accountId.substring(0, 8)}... ` +
    ...
    async handleGroupAdderBatch(campaignId, accountId, contacts, state, campaign) {
    ...
    `);
        let accountIds = accountsStmt.all(campaignId).map(row => row.account_id);
    ...
    `);
        ...
        WHERE campaign_id = ? 
          ...
          AND DATE(sent_at) = ?
      `);
            const result = sentTodayStmt.get(campaignId, accountId, today);
    ...
    getActiveCampaignState(campaignId, runId, accountId) {
        const state = this.activeCampaigns.get(campaignId);
    ...
    scheduleAccountTimeout(campaignId, accountId, delay, runId) {
    ...
    finishAccountRun(campaignId, accountId, runId) {
    ...
    async scheduleNextMessageForAccount(campaignId, accountId, runId) {
        const state = this.getActiveCampaignState(campaignId, runId, accountId);
        if (!state) {
    ...
    claimPendingContacts(campaignId, accountId, campaign, limit = 1) {
        let contactQuery = `
    ...
    `;
      ...
      `);
            const contacts = contactStmt.all(campaignId);
            if (!contacts.length) {
                return [];
            }
    ...
    async sendNextMessageFromAccount(campaignId, accountId, runId) {
        const campaign = this.getCampaign(campaignId);
        if (!campaign) {
      ...
      `);
        ...
        `);
                const debugResult = debugStmt.get(cutoffDate, contact.phone_number, contact.phone_number, contact.phone_number);
      ...
      `);
            const blacklistedResult = blacklistedStmt.get(campaignId);
        ...
        `);
                const recentContactsResult = recentContactsStmt.get(campaignId, cutoffDate);
          ...
          `);
                    markRecentStmt.run(`Skipped - received message in last ${campaign.skip_recent_days} days`, campaignId, cutoffDate);
                }
            }
      ...
      `);
            const stuckResult = stuckStmt.get(campaignId);
    ...
    `);
        const contactDetails = contactDetailsStmt.get(contact.phone_number);
      ...
      `);
          ...
          WHERE id = ?
        `);
                updateStmt.run(error.message, contact.id);
                this.notifyRenderer('campaign:progress', campaignId, {
                    contactId: contact.id,
                    status: 'failed',
                    error: error.message
            ...
            WHERE id = ?
          `);
                    updateStmt.run(`Failed after 2 attempts: ${error.message}`, newRetryCount, contact.id);
                    this.notifyRenderer('campaign:progress', campaignId, {
                        contactId: contact.id,
                        status: 'failed',
                        error: `Failed after 2 attempts: ${error.message}`
                    });
                }
                else {
    ...
    getMessageVariantPool(campaign) {
        let extraVariants = [];
        ...
        const pool = [campaign.message, ...extraVariants].filter((variant) => typeof variant === 'string' && variant.trim().length > 0);
    ...
    `);
      ...
      `);
            const recentResult = markRecentStmt.run(`Skipped - received message in last ${campaign.skip_recent_days} days`, campaignId, cutoffDate);
      ...
      WHERE campaign_id = ? AND status = 'pending'
    `);
        const finalCheck = finalCheckStmt.get(campaignId);
    ...
    resetDailyCountersIfNeeded(campaignId) {
        const state = this.activeCampaigns.get(campaignId);
        if (!state)
            return;
        const today = new Date().toISOString().split('T')[0];
