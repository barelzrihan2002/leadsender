...
export class GroupCampaignScheduler {
    constructor(db, whatsappManager) {
        ...
        this.runningCampaignIds = new Set();
        this.db = db;
    ...
    async runCampaignNow(campaignId) {
        ...
        try {
            ...
            const campaign = campaignStmt.get(campaignId);
            ...
            const allTargets = targetsStmt.all(campaignId);
      ...
      `);
            const alreadySentGroupIds = new Set(alreadySentStmt.all(campaignId, today).map(r => r.group_id));
      ...
      `);
            for (let i = 0; i < targets.length; i++) {
                ...
                const liveCampaign = campaignStmt.get(campaignId);
                ...
                try {
                    if (campaign.media_path) {
                        await this.whatsappManager.sendMedia(campaign.account_id, target.group_id, campaign.media_path, campaign.media_caption || campaign.message || '');
                    }
                    else {
                        await this.whatsappManager.sendMessage(campaign.account_id, target.group_id, campaign.message || '');
                    }
            ...
            const finalCampaign = campaignStmt.get(campaignId);
