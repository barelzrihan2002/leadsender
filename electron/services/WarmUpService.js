...
export class WarmUpService {
    ...
    resumeActiveSessions() {
      ...
      `);
            ...
            `);
              ...
              WHERE session_id = ?
            `);
                        const accountIds = accountsStmt.all(session.id).map(row => row.account_id);
                            ...
                            if (currentHour >= 8 && currentHour < 22) {
                                ...
                                accountIds.forEach((accountId, index) => {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        this.scheduleNextWarmUpForAccount(session.id, accountIds, accountId, session.min_delay, session.max_delay);
                                    }, initialDelay);
                                });
                            }
    ...
    async startSession(accountIds, minDelay, maxDelay) {
        ...
        const sessionStmt = this.db.prepare(`
      INSERT INTO warmup_sessions (id, status, min_delay, max_delay, started_at)
      VALUES (?, 'active', ?, ?, ?)
    `);
        sessionStmt.run(sessionId, minDelay, maxDelay, now);
        // Add accounts to session
        const accountStmt = this.db.prepare(`
      INSERT INTO warmup_accounts (session_id, account_id)
      VALUES (?, ?)
    `);
        const insertAccounts = this.db.transaction((ids) => {
    ...
    `);
        const session = sessionStmt.get(activeSessionId);
        if (!session) {
            return null;
        }
        const accountsStmt = this.db.prepare(`
    ...
    `);
        const accounts = accountsStmt.all(activeSessionId).map(row => row.account_id);
    ...
    async scheduleNextWarmUpForAccount(sessionId, allAccountIds, accountId, minDelay, maxDelay) {
        const state = this.activeSessions.get(sessionId);
    ...
    getPairKey(accountA, accountB) {
        return [accountA, accountB].sort().join(':');
    }
    linkConversation(state, accountA, accountB) {
        if (!state.activeConversations.has(accountA))
            state.activeConversations.set(accountA, new Set());
        if (!state.activeConversations.has(accountB))
            state.activeConversations.set(accountB, new Set());
        state.activeConversations.get(accountA).add(accountB);
        state.activeConversations.get(accountB).add(accountA);
    }
    releaseConversation(sessionId, accountA, accountB) {
        const state = this.activeSessions.get(sessionId);
        if (state) {
     ...
     */
    async startConversation(sessionId, allAccountIds, starterId) {
        const state = this.activeSessions.get(sessionId);
        if (!state) {
        ...
        const starterPartners = state.activeConversations.get(starterId) || new Set();
        const availablePartners = allAccountIds.filter(id => {
            if (id === starterId)
                return false;
            if (!this.whatsappManager.isConnected(id))
                return false;
            if (starterPartners.has(id))
                return false; // already chatting with this one
     ...
     */
    async runConversationTurn(sessionId, allAccountIds, senderId, receiverId, turnNumber, totalTurns) {
        const state = this.activeSessions.get(sessionId);
        if (!state || !state.isRunning) {
     ...
     */
    async sendWarmUpTurnMessage(sessionId, senderId, receiverId, message) {
        const state = this.activeSessions.get(sessionId);
        if (!state) {
            console.log(`🔥 Session ${sessionId} not found in active sessions`);
            return;
        ...
        const receiver = stmt.get(receiverId);
        ...
        try {
          ...
          INSERT INTO warmup_messages (id, session_id, from_account_id, to_account_id, message_text, sent_at)
          VALUES (?, ?, ?, ?, ?, ?)
        `);
                logStmt.run(uuidv4(), sessionId, senderId, receiverId, message, new Date().toISOString());
            }
            catch (e) {
                console.error('Failed to log warmup message:', e);
            }
    ...
    resetDailyCountersIfNeeded(sessionId) {
        const state = this.activeSessions.get(sessionId);
        if (!state)
            return;
