...
interface DuoPlusApiResponse<T> {
  code: number;
  data: T;
  message: string;
}
...
export class DuoPlusManager {
  ...
  private async throttle(): Promise<void> {
    ...
    this.requestQueueTail = new Promise<void>(resolve => { releaseNext = resolve; });
  ...
  getSettings(): DuoPlusSettings {
    const rows = this.db.prepare(`SELECT key, value FROM settings WHERE key LIKE 'duoplus_%'`).all() as Array<{ key: string; value: string }>;
    const map = new Map(rows.map(row => [row.key, row.value]));

    return {
      apiKey: map.get('duoplus_api_key') || '',
      tapMode: (map.get('duoplus_tap_mode') as DuoPlusTapMode) || 'auto',
  ...
  private static readonly RATE_LIMIT_CODE = 160004;
  private static readonly RATE_LIMIT_MAX_RETRIES = 5;
  private static readonly RATE_LIMIT_BASE_DELAY_MS = 1500;
  private static readonly RATE_LIMIT_JITTER_MS = 1000;
  ...
  private async request<T>(path: string, body: Record<string, unknown>): Promise<T> {
    ...
    for (let attempt = 0; attempt <= DuoPlusManager.RATE_LIMIT_MAX_RETRIES; attempt++) {
      ...
      const json = (await response.json()) as DuoPlusApiResponse<T>;
  ...
  async getCloudPhoneStatus(deviceIds: string[]): Promise<DuoPlusStatusItem[]> {
    const data = await this.request<{ list: DuoPlusStatusItem[] }>('/api/v1/cloudPhone/status', {
      image_ids: deviceIds,
    });
    return data.list || [];
  }

  async powerOn(deviceIds: string[]): Promise<void> {
    await this.request('/api/v1/cloudPhone/powerOn', { image_ids: deviceIds });
  }

  async powerOff(deviceIds: string[]): Promise<void> {
  ...
  async waitUntilOnline(deviceId: string, timeoutMs: number = 90000, pollIntervalMs: number = 5000): Promise<boolean> {
  ...
  async ensureOnline(deviceId: string, timeoutMs: number = 90000): Promise<boolean> {
    const [status] = await this.getCloudPhoneStatus([deviceId]);
  ...
  async executeAdbCommand(deviceId: string, command: string): Promise<{ success: boolean; content: string; message: string }> {
  ...
  private static readonly UI_DUMP_PATH = '/sdcard/uidump.xml';

  /** Dumps the current screen's UI hierarchy via `uiautomator` and returns the raw XML content. */
  private async dumpUiXml(deviceId: string): Promise<string> {
   ...
   */
  private findSendButtonCenter(xml: string): { x: number; y: number } | null {
   ...
   */
  async sendWhatsAppTextViaIntent(deviceId: string, phone: string, text: string): Promise<void> {
    const settings = this.getSettings();
    const sanitizedPhone = phone.replace(/[^\d]/g, '');
    const encodedText = encodeURIComponent(text);

    const openIntentCommand =
      `am start -a android.intent.action.VIEW -d "whatsapp://send?phone=${sanitizedPhone}&text=${encodedText}" com.whatsapp`;
    ...
    if (settings.tapMode === 'coordinates' && settings.tapX !== null && settings.tapY !== null) {
      ...
      if (!center) {
        throw new Error(
          'Could not locate the WhatsApp send button on screen (UI Automator dump found no match). ' +
