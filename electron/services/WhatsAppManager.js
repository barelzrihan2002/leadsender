...
async function getOrDownloadChromium() {
...
export class WhatsAppManager {
    constructor(db) {
        ...
        this.sessionsPaths = new Map();
        this.readyAccounts = new Set(); // Track which accounts are fully ready
        this.connectingAccounts = new Set();
        this.anonymizedProxyServers = new Map(); // accountId -> proxy server
        ...
        try {
            const countStmt = this.db.prepare("SELECT COUNT(*) as count FROM accounts WHERE status IN ('connected', 'connecting', 'qr') OR session_path IS NOT NULL");
    ...
    setCampaignScheduler(scheduler) {
     ...
     */
    waitForAccountReady(accountId, timeoutMs = 120000) {
    ...
    isTrackedClient(accountId, client) {
    ...
    async cleanupProxyServer(accountId) {
    ...
    async destroyClientInstance(accountId, client) {
    ...
    async cleanupAccountResources(accountId, client) {
     ...
     */
    setupPairingDebugLogging(accountId, client) {
            ...
            try {
                ...
                try {
                    const diagnostic = await page.evaluate(() => {
                        ...
                        if (hasRequire) {
                            try {
                                hasAltLinkingModule = !!window.require('WAWebAltDeviceLinkingApi');
                            }
                            catch (e) {
    ...
    requestPairingCodeAfterInitialize(accountId, client, phoneNumber, intervalMs = 180000) {
        ...
        (async () => {
            try {
                ...
                await page.waitForFunction(() => {
                    ...
                    return Boolean(typeof window.Debug?.VERSION !== 'undefined' &&
                ...
                const socketState = await page.evaluate(() => window.require?.('WAWebSocketModel')?.Socket?.state || null);
     ...
     */
    grantWebPermissions(accountId, client) {
        (async () => {
            try {
                let browser = null;
    ...
    async connectAccount(accountId, proxy, pairingMethod = 'qr') {
        ...
        try {
                ...
                if (proxy.username && proxy.password) {
                    ...
                    try {
                        ...
                        if (match) {
                            ...
                            const server = proxyChain.servers?.get(localPort);
                            if (server) {
            ...
            if (pairingMethod === 'code' && phoneNumber) {
                ...
                (async () => {
                    ...
                    try {
                        const directResult = await page.evaluate(async (phoneNum, showNotif) => {
                            try {
                                ...
                                try {
                                    for (const key of Object.getOwnPropertyNames(e)) {
                                        ...
                                        catch { /* skip unserializable prop */ }
                                    }
                                }
                                catch { /* ignore */ }
                                let serialized = '';
                                try {
                                    serialized = JSON.stringify(ownProps);
                                }
                                catch {
                                    serialized = '(not JSON-serializable)';
                                }
                                return {
                                    success: false,
                                    name: e?.name,
            ...
            try {
                ...
                if (error?.stack)
                    console.error(`${pairingTag} ❌ Stack:`, error.stack);
                await this.cleanupAccountResources(accountId, client);
                throw error;
            }
        }
    ...
    async sendMessage(accountId, to, message, isWarmup = false) {
        ...
        const myNumber = client.info?.wid?._serialized?.split('@')[0] || '';
        const myName = client.info?.pushname || null;
    ...
    async sendMedia(accountId, to, filePath, caption) {
        const client = this.clients.get(accountId);
        if (!client) {
            throw new Error('Account not connected');
        }
    ...
    async updateWhatsAppName(accountId, name) {
        const client = this.clients.get(accountId);
        if (!client) {
            throw new Error('Account not connected');
        }
    ...
    async sendMediaFromPath(accountId, to, filePath, caption, isWarmup = false) {
    ...
    async refreshProfilePicture(accountId) {
        const client = this.clients.get(accountId);
        if (!client) {
            throw new Error('Account not connected');
        }
    ...
    async updateWhatsAppProfilePicture(accountId, imagePath) {
        const client = this.clients.get(accountId);
        if (!client) {
            throw new Error('Account not connected');
        }
    ...
    async resolvePhoneFromHistory(accountId, chatId) {
        const client = this.clients.get(accountId);
        const myNumber = this.extractDigits(client?.info?.wid?._serialized || '');
        try {
    ...
    async resolvePhoneNumber(accountId, identifier, message) {
        const client = this.clients.get(accountId);
        ...
        for (const candidate of candidates) {
            ...
            if (client && typeof client.getContactLidAndPhone === 'function') {
                try {
                    const result = await client.getContactLidAndPhone([candidate]);
    ...
    async resolveContactData(accountId, identifier, message) {
        ...
        if (phone) {
            try {
                ...
                const dbContact = this.db.prepare('SELECT name FROM contacts WHERE phone_number = ? OR phone_number LIKE ?').get(phone, `%${suffix}`);
                if (dbContact?.name) {
    ...
    async normalizeSendTarget(accountId, to) {
        if (!to.includes('@')) {
    ...
    isConnected(accountId) {
        const client = this.clients.get(accountId);
        const hasClient = client && client.pupPage != null;
     ...
     */
    async waitForOutgoingMessage(accountId, phoneNumber, afterTimestampMs, timeoutMs = 25000, pollIntervalMs = 2000) {
    ...
    mapAddParticipantResultCode(rawCode, isInviteV4Sent) {
        if (rawCode === 200) {
            return 'added';
        }
        if (rawCode === 403) {
    ...
    async getGroupAccessState(accountId, groupId) {
        const client = this.clients.get(accountId);
        ...
        const accessState = await page.evaluate(async (targetGroupId) => {
            ...
            return {
                ...
                isAdmin: participants.some((participant) => participant.id === meSerialized && participant.isAdmin)
            };
        }, groupId);
        return {
            chat: null,
    ...
    async canAddParticipantsToGroup(accountId, groupId) {
        try {
            ...
            const message = error.message || 'Failed to access group';
        ...
        try {
            ...
            const groups = await page.evaluate(async () => {
                ...
                const meUser = window.require('WAWebUserPrefsMeUser').getMaybeMePnUser?.() ||
                    window.require('WAWebUserPrefsMeUser').getMaybeMeLidUser?.();
                const meSerialized = meUser?._serialized || meUser?.toString?.() || null;
                const groupsOnly = chats.filter((chat) => Boolean(chat?.groupMetadata));
                return await Promise.all(groupsOnly.map(async (chat) => {
                    try {
    ...
    normalizeGroupInviteInfo(rawInfo, inviteCode) {
    ...
    async getGroupInviteInfo(accountId, inviteLink) {
        const client = this.clients.get(accountId);
        if (!client) {
            throw new Error('Account not connected');
        }
        ...
        try {
            ...
            const message = error.message || 'Failed to get invite info';
            const loweredMessage = message.toLowerCase();
            if (loweredMessage.includes('not connected')) {
                throw new Error('Account not connected');
            }
    ...
    async getGroupParticipants(accountId, groupId) {
        ...
        try {
            ...
            const participants = await page.evaluate(async (targetGroupId) => {
                const chatCollection = window.require('WAWebCollections').Chat;
                const groupMetadataCollection = window.require('WAWebCollections').GroupMetadata ||
                    window.require('WAWebCollections').WAWebGroupMetadataCollection;
                const widFactory = window.require('WAWebWidFactory');
     ...
     */
    async acceptInviteRaw(client, inviteCode) {
        return client.pupPage.evaluate(async (code) => {
            try {
    ...
    async joinGroupByInviteLink(accountId, inviteLink) {
        const client = this.clients.get(accountId);
        if (!client) {
            return {
                success: false,
     ...
     */
    async addParticipantRaw(client, groupId, participantId, options = {}) {
        return client.pupPage.evaluate(async (gId, pId, opts) => {
            ...
            const groupWid = window.require('WAWebWidFactory').createWid(gId);
            ...
            let pWid = window.require('WAWebWidFactory').createWid(pId);
            if (groupParticipants.some((p) => p._serialized === pWid._serialized)) {
            ...
            if (autoSendInviteV4 && code === 403) {
                ...
                if (rpcResult.name === 'ParticipantRequestCodeCanBeSent') {
                    const userChat = window.require('WAWebCollections').Chat.get(pWid) ||
                    ...
                    if (userChat) {
                        ...
                        const res = await window
                            ...
                            .sendGroupInviteMessage(userChat, group.id._serialized, groupName, rpcResult.inviteV4Code, rpcResult.inviteV4CodeExp, comment, await window.WWebJS.getProfilePicThumbToBase64(groupWid));
                        isInviteV4Sent = res.messageSendResult === 'OK';
                    }
                }
            }
            return { code, message: errorCodes[code] || errorCodes.default, isInviteV4Sent };
        }, groupId, participantId, options);
    }
    async addParticipantToGroup(accountId, groupId, phoneNumber) {
        try {
            ...
            const message = error.message || 'Failed to add participant to group';
            const loweredMessage = message.toLowerCase();
            let resultCode = 'unknown_error';
            if (loweredMessage.includes('not connected')) {
                resultCode = 'account_not_connected';
            }
    ...
    updateAccountStatus(accountId, status, qrCode) {
    ...
    async handleIncomingMessage(accountId, msg) {
        try {
            ...
            const myNumber = this.clients.get(accountId)?.info?.wid?._serialized?.split('@')[0] || '';
        ...
        `);
            ...
            if (isFromMe) {
                ...
                senderName = this.clients.get(accountId)?.info?.pushname || null;
                if (resolvedToNumber) {
            ...
            const client = this.clients.get(accountId);
            const softwareChatId = await this.chatManager.findOrCreateChat(otherNumber, accountId, client, isFromMe ? null : senderName);
            this.chatManager.updateLastMessageAt(softwareChatId, timestamp);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(messageId, accountId, chatId, fromNumber, toNumber, senderName, msg.body || null, messageType, mediaFilename, mediaMimetype, isFromMeValue, isRead, softwareChatId, messageType, timestamp);
