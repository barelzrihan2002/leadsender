...
let inboxManager;
let licenseManager;
let scheduledCampaignChecker;
let flowEngine;
let groupCampaignScheduler;
let duoplusManager;
...
function logActivity(db, type, message, relatedId) {
    try {
...
function parseSourceTagIds(sourceTagIds) {
    if (Array.isArray(sourceTagIds)) {
        return sourceTagIds.filter((tagId) => typeof tagId === 'string' && tagId.trim().length > 0);
    }
    ...
    try {
        ...
        return Array.isArray(parsed)
            ? parsed.filter((tagId) => typeof tagId === 'string' && tagId.trim().length > 0)
            : [];
    }
    catch {
        return [];
...
function serializeSourceTagIds(sourceTagIds) {
    const normalized = parseSourceTagIds(sourceTagIds);
    return normalized.length > 0 ? JSON.stringify([...new Set(normalized)]) : null;
}
function parseMessageVariants(messageVariants) {
    if (Array.isArray(messageVariants)) {
        return messageVariants.filter((variant) => typeof variant === 'string' && variant.trim().length > 0);
    }
    ...
    try {
        ...
        return Array.isArray(parsed)
            ? parsed.filter((variant) => typeof variant === 'string' && variant.trim().length > 0)
            : [];
    }
    catch {
        return [];
...
function serializeMessageVariants(messageVariants) {
    const normalized = parseMessageVariants(messageVariants);
    return normalized.length > 0 ? JSON.stringify(normalized) : null;
}
function normalizeCampaignRow(campaign) {
    if (!campaign) {
        return null;
    }
    return {
...
export function setupIPCHandlers() {
    ...
    ipcMain.handle('accounts:create', async (_event, data) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, data.phone_number, data.name || null, data.proxy_host || null, data.proxy_port || null, data.proxy_username || null, data.proxy_password || null, 'http' // Always HTTP - SOCKS5 not supported
        );
    ...
    ipcMain.handle('accounts:connect', async (_event, id, proxy, pairingMethod = 'qr') => {
        const accountStmt = db.prepare('SELECT name, phone_number FROM accounts WHERE id = ?');
        const account = accountStmt.get(id);
    ...
    ipcMain.handle('accounts:disconnect', async (_event, id) => {
        await whatsappManager.disconnectAccount(id);
    });
    ipcMain.handle('accounts:getQRCode', async (_event, id) => {
        return await whatsappManager.getQRCode(id);
    });
    ipcMain.handle('accounts:updateWhatsAppName', async (_event, id, name) => {
        await whatsappManager.updateWhatsAppName(id, name);
    });
    ipcMain.handle('accounts:updateWhatsAppImage', async (_event, id, imagePath) => {
        await whatsappManager.updateWhatsAppProfilePicture(id, imagePath);
    });
    ipcMain.handle('accounts:refreshProfilePicture', async (_event, id) => {
        await whatsappManager.refreshProfilePicture(id);
    });
    ipcMain.handle('accounts:setDuoplusDeviceId', async (_event, id, deviceId) => {
    ...
    ipcMain.handle('duoplus:saveSettings', async (_event, settings) => {
        duoplusManager.saveSettings(settings);
    });
    ipcMain.handle('duoplus:testDevice', async (_event, deviceId) => {
    ...
    ipcMain.handle('campaigns:getAll', async () => {
        ...
        return stmt.all().map(campaign => normalizeCampaignRow(campaign));
    });
    ipcMain.handle('campaigns:getById', async (_event, id) => {
    ...
    ipcMain.handle('campaigns:create', async (_event, data) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, data.name, data.message ?? '', data.campaign_type || 'message', data.min_delay || 30, data.max_delay || 60, data.max_messages_per_day || 100, data.start_hour || 9, data.end_hour || 18, mediaPath, mediaType, mediaCaption, data.scheduled_start_datetime || null, data.messages_before_break || null, data.break_duration || null, data.skip_recent_contacts ? 1 : 0, data.skip_recent_days || 7, data.target_group_id || null, data.target_group_name || null, data.group_source_account_id || null, serializeSourceTagIds(data.source_tag_ids), serializeMessageVariants(data.message_variants), sendMode);
    ...
    ipcMain.handle('campaigns:save-media', async (_event, fileName, buffer) => {
        try {
    ...
    ipcMain.handle('campaigns:update', async (_event, id, data) => {
        const updates = [];
        const values = [];
        // Basic fields
        if (data.name !== undefined) {
        ...
        if (updates.length > 0) {
            ...
            const campaign = campaignStmt.get(id);
    ...
    ipcMain.handle('campaigns:exportReport', async (_event, campaignId) => {
        const db = getDatabase();
    ...
    ipcMain.handle('campaigns:addContacts', async (_event, id, contacts) => {
        const stmt = db.prepare(`
    ...
    `);
        const uniqueContacts = Array.from(new Set(contacts
            ...
            .filter((phoneNumber) => Boolean(phoneNumber)))).map(phone_number => ({ phone_number }));
    ...
    ipcMain.handle('campaigns:getContacts', async (_event, id) => {
        const stmt = db.prepare('SELECT * FROM campaign_contacts WHERE campaign_id = ?');
        return stmt.all(id);
    });
    ipcMain.handle('campaigns:addAccounts', async (_event, campaignId, accountIds) => {
        const stmt = db.prepare(`
    ...
    ipcMain.handle('campaigns:setAccounts', async (_event, campaignId, accountIds) => {
        const deleteStmt = db.prepare(`DELETE FROM campaign_accounts WHERE campaign_id = ?`);
        const insertStmt = db.prepare(`
      INSERT OR IGNORE INTO campaign_accounts (campaign_id, account_id)
      VALUES (?, ?)
    `);
        const replaceAccounts = db.transaction((ids) => {
    ...
    ipcMain.handle('campaigns:setContacts', async (_event, campaignId, contacts) => {
    ...
    `);
        const uniqueContacts = Array.from(new Set(contacts
            .map(contact => contact.phone_number?.trim())
            .filter((phoneNumber) => Boolean(phoneNumber)))).map(phone_number => ({ phone_number }));
        const replaceContacts = db.transaction((contactList) => {
            deleteStmt.run(campaignId);
            for (const contact of contactList) {
    ...
    ipcMain.handle('campaigns:getAccounts', async (_event, campaignId) => {
        const stmt = db.prepare('SELECT account_id FROM campaign_accounts WHERE campaign_id = ?');
        return stmt.all(campaignId).map((row) => row.account_id);
    });
    // ==================== CONTACT HANDLERS ====================
    ...
    `);
        ...
        return contacts.map(contact => ({
            ...
            tags: contact.tags_data ? contact.tags_data.split(',').map((t) => {
    ...
    ipcMain.handle('contacts:getById', async (_event, id) => {
    ...
    ipcMain.handle('contacts:previewFile', async (_event, filePath, country = 'international') => {
        try {
            let data;
            ...
            function normalizePhone(phone, countryCode) {
                ...
                if (countryCode === 'israel') {
                    ...
                    if (cleaned.startsWith('05') && cleaned.length === 10)
                        return '9725' + cleaned.substring(2);
            ...
            function validatePhonePreview(normalized, countryCode) {
    ...
    ipcMain.handle('contacts:isInBlacklist', async (_event, phoneNumber) => {
        const phoneVariants = normalizePhoneForMatching(phoneNumber);
        // Build dynamic query with all variants
        const placeholders = phoneVariants.map(() => 'REPLACE(REPLACE(REPLACE(c.phone_number, \'-\', \'\'), \' \', \'\'), \'+\', \'\') = ?').join(' OR ');
        const stmt = db.prepare(`
      ...
      WHERE t.name = 'BlackList'
        AND (${placeholders})
    `);
        const result = stmt.get(...phoneVariants);
        return (result?.count || 0) > 0;
    });
    // Find contact by phone number (checks all formats)
    ipcMain.handle('contacts:findByPhone', async (_event, phoneNumber) => {
        const phoneVariants = normalizePhoneForMatching(phoneNumber);
        // Build dynamic query with all variants
        const placeholders = phoneVariants.map(() => 'REPLACE(REPLACE(REPLACE(phone_number, \'-\', \'\'), \' \', \'\'), \'+\', \'\') = ?').join(' OR ');
        const stmt = db.prepare(`
      SELECT c.*, GROUP_CONCAT(t.id || ':' || t.name || ':' || COALESCE(t.color, '')) as tags_data
      FROM contacts c
      LEFT JOIN contact_tags ct ON c.id = ct.contact_id
      ...
      LIMIT 1
    `);
        const contact = stmt.get(...phoneVariants);
        if (!contact)
            return null;
        return {
            ...contact,
    ...
    ipcMain.handle('contacts:checkDuplicates', async (_event, filePath, country = 'international') => {
        try {
            let data;
            if (filePath.toLowerCase().endsWith('.csv')) {
                let csvContent = fs.readFileSync(filePath, 'utf8');
            ...
            function normalizePhone(phone, countryCode) {
                let phoneStr = String(phone);
                if (phoneStr.includes('E+') || phoneStr.includes('e+'))
                    phoneStr = String(Math.round(Number(phone)));
                let cleaned = phoneStr.trim().replace(/[^\d+]/g, '').replace(/^\+/, '');
                if (countryCode === 'israel') {
                    if (cleaned.startsWith('972') && cleaned.length >= 12)
                        return cleaned;
            ...
            const existingPhones = new Set(existingPhonesStmt.all().map(row => row.phone_number));
    ...
    ipcMain.handle('contacts:importFromFile', async (_event, filePath, country, duplicateAction) => {
        ...
        function normalizePhone(phone, countryCode = 'international') {
            let phoneStr = String(phone);
        ...
        function validatePhone(normalized, countryCode) {
            if (!normalized || normalized.length === 0)
                return false;
            switch (countryCode) {
                case 'israel':
        ...
        const customFields = customFieldsStmt.all();
        // Pre-cache ALL existing contacts in memory (avoids per-row DB lookups)
        const existingContactsCache = new Map();
        const allContacts = db.prepare('SELECT id, phone_number, name, custom_fields FROM contacts').all();
    ...
    ipcMain.handle('contacts:addTag', async (_event, contactId, tagId) => {
        const stmt = db.prepare(`
    ...
    ipcMain.handle('contacts:removeTag', async (_event, contactId, tagId) => {
    ...
    ipcMain.handle('tags:create', async (_event, data) => {
        const id = uuidv4();
        const stmt = db.prepare(`
    ...
    ipcMain.handle('tags:update', async (_event, id, data) => {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
            updates.push('name = ?');
    ...
    ipcMain.handle('extractor:getGroups', async (_event, accountId) => {
        return await whatsappManager.getGroups(accountId);
    });
    ipcMain.handle('extractor:getGroupParticipants', async (_event, accountId, groupId) => {
        return await whatsappManager.getGroupParticipants(accountId, groupId);
    });
    ipcMain.handle('groups:getGroups', async (_event, accountId) => {
        return await whatsappManager.getGroups(accountId);
    });
    ipcMain.handle('groups:getGroupParticipants', async (_event, accountId, groupId) => {
        return await whatsappManager.getGroupParticipants(accountId, groupId);
    });
    ipcMain.handle('groups:getInviteInfo', async (_event, accountId, inviteLink) => {
        return await whatsappManager.getGroupInviteInfo(accountId, inviteLink);
    });
    ipcMain.handle('groups:joinGroupByInviteLink', async (_event, accountId, inviteLink) => {
        return await whatsappManager.joinGroupByInviteLink(accountId, inviteLink);
    });
    // ==================== GROUP CAMPAIGNS HANDLERS ====================
    function normalizeGroupCampaignRow(row) {
        if (!row)
            return null;
        let days = [];
        try {
    ...
    ipcMain.handle('groupCampaigns:getAll', async () => {
        ...
        return stmt.all().map(normalizeGroupCampaignRow);
    });
    ipcMain.handle('groupCampaigns:getById', async (_event, id) => {
        const stmt = db.prepare('SELECT * FROM group_campaigns WHERE id = ?');
        return normalizeGroupCampaignRow(stmt.get(id));
    });
    ipcMain.handle('groupCampaigns:getTargets', async (_event, id) => {
        const stmt = db.prepare('SELECT group_id, group_name FROM group_campaign_targets WHERE campaign_id = ?');
        return stmt.all(id);
    });
    ipcMain.handle('groupCampaigns:getRuns', async (_event, id) => {
    ...
    ipcMain.handle('groupCampaigns:create', async (_event, data) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
    `);
        stmt.run(id, data.name, data.account_id, data.message || null, data.media_path || null, data.media_type || null, data.media_caption || null, JSON.stringify(data.days_of_week || []), data.send_hour, data.send_minute ?? 0, data.min_delay ?? 20, data.max_delay ?? 60);
        const insertTargetStmt = db.prepare(`
      INSERT OR IGNORE INTO group_campaign_targets (campaign_id, group_id, group_name)
      VALUES (?, ?, ?)
    `);
        const insertTargets = db.transaction((targets) => {
    ...
    ipcMain.handle('groupCampaigns:update', async (_event, id, data) => {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
            updates.push('name = ?');
        ...
        if (data.send_hour !== undefined) {
            ...
            values.push(data.send_hour);
        }
        if (data.send_minute !== undefined) {
            updates.push('send_minute = ?');
            values.push(data.send_minute);
        }
        if (data.min_delay !== undefined) {
            updates.push('min_delay = ?');
            values.push(data.min_delay);
    ...
    ipcMain.handle('groupCampaigns:delete', async (_event, id) => {
        db.prepare('DELETE FROM group_campaigns WHERE id = ?').run(id);
    });
    ipcMain.handle('groupCampaigns:start', async (_event, id) => {
        const campaign = db.prepare('SELECT name FROM group_campaigns WHERE id = ?').get(id);
    ...
    ipcMain.handle('groupCampaigns:pause', async (_event, id) => {
        const campaign = db.prepare('SELECT name FROM group_campaigns WHERE id = ?').get(id);
    ...
    ipcMain.handle('groupCampaigns:stop', async (_event, id) => {
        const campaign = db.prepare('SELECT name FROM group_campaigns WHERE id = ?').get(id);
    ...
    ipcMain.handle('customFields:create', async (_event, data) => {
        const id = uuidv4();
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, data.name, data.label, data.type || 'text', data.required ? 1 : 0, nextOrder);
        const getStmt = db.prepare('SELECT * FROM custom_fields WHERE id = ?');
        return getStmt.get(id);
    });
    ipcMain.handle('customFields:update', async (_event, id, data) => {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
            updates.push('name = ?');
    ...
    // ==================== MESSAGE HANDLERS ====================
    ipcMain.handle('messages:send', async (_event, accountId, to, message) => {
    ...
    ipcMain.handle('messages:sendMedia', async (_event, accountId, to, filePath, caption) => {
        await whatsappManager.sendMedia(accountId, to, filePath, caption);
    });
    ipcMain.handle('messages:saveTempFile', async (_event, fileName, buffer) => {
    ...
    ipcMain.handle('messages:deleteTempFile', async (_event, filePath) => {
        try {
    ...
    ipcMain.handle('messages:getByChat', async (_event, softwareChatId) => {
        const stmt = db.prepare(`
    ...
    `);
        const messages = stmt.all(softwareChatId);
        return messages;
    });
    ipcMain.handle('messages:getChats', async (_event, accountId, searchQuery) => {
    ...
    `;
        ...
        const chats = chatsStmt.all(...params);
    ...
    `);
        ...
        const validChats = mappedChats.filter(chat => {
            ...
            if (!messageText || messageText.trim() === '')
                return false;
            if (messageText.trim().toLowerCase() === 'cc')
                return false;
            return true;
        });
        // Apply search filter
    ...
    ipcMain.handle('messages:markAsRead', async (_event, softwareChatId) => {
        const stmt = db.prepare(`
      UPDATE messages 
      SET is_read = 1 
    ...
    ipcMain.handle('messages:markAsHandled', async (_event, chatId, accountId) => {
        const stmt = db.prepare(`
      UPDATE messages 
      SET is_handled = 1 
    ...
    ipcMain.handle('messages:markChatStatus', async (_event, softwareChatId, status) => {
    ...
    ipcMain.handle('messages:markAllChats', async (_event, accountId, handled) => {
    ...
    ipcMain.handle('messages:getChatPhoto', async (_event, photoPath) => {
        try {
    ...
    ipcMain.handle('messages:getMediaFile', async (_event, messageId) => {
        try {
            const message = db.prepare('SELECT media_filename FROM messages WHERE id = ?').get(messageId);
    ...
    // ==================== WARMUP HANDLERS ====================
    ipcMain.handle('warmup:start', async (_event, accountIds, minDelay, maxDelay) => {
        return await warmUpService.startSession(accountIds, minDelay, maxDelay);
    });
    ipcMain.handle('warmup:stop', async (_event, sessionId) => {
    ...
    ipcMain.handle('warmup:getLogs', async (_event, limit = 50) => {
        const db = getDatabase();
    ...
    ipcMain.handle('warmup:getSessionStats', async (_event, sessionId) => {
        const db = getDatabase();
    ...
    ipcMain.handle('warmup:getSessionDetails', async (_event, sessionId) => {
        const db = getDatabase();
        // Get session info
    ...
    ipcMain.handle('stats:getDashboard', async () => {
        ...
        const accountsConnected = accountsStmt.get().count;
    ...
    ipcMain.handle('stats:getRecentActivities', async (_event, limit = 10) => {
        const stmt = db.prepare(`
    ...
    function buildDailyRange(startDate, endDate) {
        const days = [];
    ...
    function queryCampaignContactStats(campaignType, startDate, endDate, accountId, campaignId) {
        const conditions = [
            "c.campaign_type = ?",
            "cc.sent_at IS NOT NULL",
            "DATE(cc.sent_at) BETWEEN ? AND ?"
        ];
        const params = [campaignType, startDate, endDate];
        if (accountId) {
    ...
    `).all(...params);
        ...
        for (const row of rows) {
            ...
            if (!dailyMap.has(day))
                dailyMap.set(day, { sent: 0, failed: 0 });
    ...
    ipcMain.handle('stats:getCampaignStats', async (_event, startDate, endDate, accountId, campaignId) => {
        return queryCampaignContactStats('message', startDate, endDate, accountId, campaignId);
    });
    ipcMain.handle('stats:getGroupAdderStats', async (_event, startDate, endDate, accountId, campaignId) => {
        return queryCampaignContactStats('group_adder', startDate, endDate, accountId, campaignId);
    });
    ipcMain.handle('stats:getGroupCampaignStats', async (_event, startDate, endDate, accountId, campaignId) => {
        const conditions = ["gcr.run_date BETWEEN ? AND ?"];
    ...
    ipcMain.handle('stats:getWarmupStats', async (_event, startDate, endDate, accountId) => {
        const conditions = ["DATE(wm.sent_at) BETWEEN ? AND ?"];
        const params = [startDate, endDate];
        if (accountId) {
    ...
    ipcMain.handle('stats:getCampaignsList', async (_event, campaignType) => {
        const rows = db.prepare(`
    ...
    ipcMain.handle('stats:exportReport', async (_event, type, startDate, endDate, accountId, campaignId) => {
        const { dialog } = await import('electron');
        let rows = [];
        let excelData = [];
    ...
    ipcMain.handle('templates:getById', async (_event, id) => {
        const db = getDatabase();
    ...
    ipcMain.handle('templates:update', async (_event, id, data) => {
      ...
      WHERE id = ?
    `);
        stmt.run(data.name, data.message, data.media_path || null, data.media_type || null, new Date().toISOString(), id);
    });
    ipcMain.handle('templates:delete', async (_event, id) => {
        const db = getDatabase();
        db.prepare('DELETE FROM message_templates WHERE id = ?').run(id);
    });
    ipcMain.handle('templates:getMediaFile', async (_event, mediaPath) => {
        try {
            if (!fs.existsSync(mediaPath)) {
    ...
    // ==================== FLOWS HANDLERS ====================
    ipcMain.handle('flows:create', async (_event, flowData) => {
        const db = getDatabase();
        const id = uuidv4();
        const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(id, flowData.name, flowData.description || null, JSON.stringify(flowData.account_ids || []), flowData.is_active !== undefined ? (flowData.is_active ? 1 : 0) : 1);
        logActivity(db, 'flow', `Flow "${flowData.name}" created`, id);
        return id;
    });
    ipcMain.handle('flows:save', async (_event, flowId, flowData) => {
        const db = getDatabase();
        const timestamp = new Date().toISOString();
        const nodes = flowData?.nodes || [];
        const edges = flowData?.edges || [];
        db.prepare('UPDATE flows SET name = ?, description = ?, account_ids = ?, updated_at = ? WHERE id = ?').run(flowData?.name, flowData?.description || null, JSON.stringify(flowData?.account_ids || []), timestamp, flowId);
    ...
    `);
        for (const node of nodes) {
            nodeStmt.run(node.id, flowId, node.type, node.position.x, node.position.y, JSON.stringify(node.data));
        }
        // הוסף edges חדשים
    ...
    `);
        for (const edge of edges) {
            edgeStmt.run(edge.id, flowId, edge.source, edge.target, edge.label || null);
        }
        const flow = db.prepare('SELECT name FROM flows WHERE id = ?').get(flowId);
    ...
    ipcMain.handle('flows:getById', async (_event, flowId) => {
        const db = getDatabase();
        const flow = db.prepare('SELECT * FROM flows WHERE id = ?').get(flowId);
        if (!flow)
            return null;
        const nodes = db.prepare('SELECT * FROM flow_nodes WHERE flow_id = ?').all(flowId);
        const edges = db.prepare('SELECT * FROM flow_edges WHERE flow_id = ?').all(flowId);
        return {
            flow: {
                ...flow,
                account_ids: flow.account_ids ? JSON.parse(flow.account_ids) : [],
    ...
    ipcMain.handle('flows:toggleActive', async (_event, flowId) => {
        const db = getDatabase();
        db.prepare('UPDATE flows SET is_active = NOT is_active, updated_at = ? WHERE id = ?')
            .run(new Date().toISOString(), flowId);
        const flow = db.prepare('SELECT name, is_active FROM flows WHERE id = ?').get(flowId);
