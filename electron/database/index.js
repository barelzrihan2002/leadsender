  ...
  `);
    ...
    try {
      ...
      `);
            ...
            const dedupeCampaignContacts = db.transaction((groups) => {
                for (const group of groups) {
                    const rows = selectDuplicatesStmt.all(group.campaign_id, group.phone_number);
  ...
  `);
    ...
    try {
      ...
      `).all();
          ...
          `).get(chat.chat_id, chat.account_id);
            ...
            `).get(chat.chat_id, chat.account_id);
                    ...
                    for (const variant of variants) {
                        contact = db.prepare(`SELECT id, name FROM contacts WHERE phone_number LIKE '%' || ? || '%'`).get(variant);
                        if (contact)
                            break;
                    }
